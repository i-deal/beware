...
def load_model():
    ...
    checkpoint = torch.load(
        f"checkpoints/run1/checkpoint.pth", device, weights_only=True
    )
...
class ViolenceCNN(nn.Module):
    ...
    def train_ep(self, dataloader, optimizer):
        ...
        for batch_idx, (data, target) in enumerate(
            tqdm(dataloader, desc="Train", unit="batch", total=max_iter)
        ):
    ...
    def test(self, dataloader):
        ...
        for batch_idx, (data, target) in enumerate(
            tqdm(dataloader, desc="Test", unit="batch", total=max_iter)
        ):
            data, target = data.to(device), target.to(device)
            if data.dim() == 5:
                data = data.squeeze(0)
    ...
    def train_full(self, train_dataloader, test_dataloader, device, checkpoint_folder):
        ...
        for epoch in range(0, 400):
            ...
            if epoch % 4 == 0:
                torch.save(
                    checkpoint, f"checkpoints/{checkpoint_folder}/checkpoint.pth"
                )
