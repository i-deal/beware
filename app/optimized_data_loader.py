...
class OptimizedDCSASSVideoDataset(Dataset):
    ...
    def _build_dataset_parallel(self, max_videos_per_crime: int) -> List[Dict]:
        ...
        log.info(f"Dataset shuffled with {len(samples)} samples")
    ...
    def _process_crime_type(self, crime_type: str) -> List[Dict]:
        ...
        if not crime_dir.exists():
            log.warning(f"Crime directory not found: {crime_dir}")
            return []

        # Get all video folders for this crime type
        video_folders = [
            d for d in crime_dir.iterdir() if d.is_dir() and d.name.endswith(".mp4")
        ]

        if not video_folders:
            log.warning(f"No video folders found for {crime_type}")
            return []

        # Randomly select ONE video folder per crime type
        selected_folder = random.choice(video_folders)

        log.info(f"Selected video folder for {crime_type}: {selected_folder.name}")

        # Get ALL clips from the selected video folder
        clips = self._get_clips_from_folder_fast(selected_folder, crime_type)
        log.info(f"  Loaded {len(clips)} clips from {selected_folder.name}")

        return clips

    def _get_clips_from_folder_fast(
        self, video_folder: Path, crime_type: str
    ) -> List[Dict]:
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int]:
        ...
        frames = self._load_video_clip_cached(sample["video_path"], sample["cache_key"])
...
class OptimizedDCSASSDataLoader:
    ...
    def __init__(
        ...
        log.info(f"Optimized data loaders created:")
        log.info(f"  Train: {len(self.train_dataset)} samples")
        log.info(f"  Val: {len(self.val_dataset)} samples")
        log.info(f"  Test: {len(self.test_dataset)} samples")
        log.info(f"  Workers: {num_workers}")
        log.info(f"  Cache: {'Enabled' if use_cache else 'Disabled'}")
    ...
    def _create_optimized_data_loader(
        self,
        dataset,
        shuffle: bool = True,
        pin_memory: bool = True,
...
def test_optimized_data_loader():
    ...
    log.info("🚀 Testing Optimized DCSASS Data Loader...")
    ...
    try:
        ...
        log.info(f"✅ Optimized dataset loaded successfully!")
        log.info(f"   Total samples: {info['total_samples']}")
        log.info(f"   Crime types: {info['num_crime_types']}")
        log.info(f"   Batch size: {info['batch_size']}")
        log.info(f"   Cache enabled: {info['cache_enabled']}")
        log.info(f"   Workers: {info['workers']}")

        # Test loading speed
        log.info("🔄 Testing batch loading speed...")
        start_time = time.time()

        for batch_idx, (videos, labels) in enumerate(data_loader.train_loader):
            log.info(f"   Batch {batch_idx + 1}: {videos.shape}, {labels.shape}")
            if batch_idx >= 2:  # Test first 3 batches
                break

        end_time = time.time()
        log.info(f"✅ Speed test completed in {end_time - start_time:.2f} seconds!")

        return True

    except Exception as e:
        log.error(f"❌ Optimized data loader test failed: {e}")
        return False
